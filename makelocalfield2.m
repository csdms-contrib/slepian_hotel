function varargout=makelocalfield2(data,rad,lat,lon,dom,Lmax,J,rplanet,avgsat,rotcoord,loadJ,savename,niter,weights,lambda,coef0)
...
if length(data)==length(rad)
    ...
    if (~ischar(dom)) & (rotcoord(1)~=0 || rotcoord(2)~=0)
        ...
        [G,~]=glmalphapotup(dom,Lmax,rplanet,avgsat);
        G = potup(G,rplanet,avgsat,Lmax,0);
    end
