function varargout=glmalphauptoJp(TH,L,rnew,rold,phi,theta,omega,J)
% [Grot,V]=glmalphauptoJp(TH,L,rnew,rold,phi,theta,omega,J)
%
% glmalphaptoJp for upward transformed scalar Slepian functions.
% Loads glmalphaup and rotates the first J functions only. 
%
% INPUT:
%
...
if length(L)==1
    if length(TH)==1
        fname=fullfile(getenv('IFILES'),'GLMALPHAUPTOJP',...
             sprintf('glmalphauptoJp-%g-%i-%g-%g-%i-%i-%i-%i.mat',...
                 TH,L,rnew,rold,phi,theta,omega,J));
    elseif length(TH)==2
        fname=fullfile(getenv('IFILES'),'GLMALPHAUPTOJP',...
             sprintf('glmalphauptoJp-%g_%g-%i-%g-%g-%i-%i-%i-%i.mat',...
...
elseif length(L)==2
    if length(TH)==1
        fname=fullfile(getenv('IFILES'),'GLMALPHAUPTOJP',...
             sprintf('glmalphauptoJp-%g-%i_%i-%g-%g-%i-%i-%i-%i.mat',...
                 TH,min(L),max(L),rnew,rold,phi,theta,omega,J));
    elseif length(TH)==2
        fname=fullfile(getenv('IFILES'),'GLMALPHAUPTOJP',...
             sprintf('glmalphauptoJp-%g_%g-%i_%i-%g-%g-%i-%i-%i-%i.mat',...
