...
if upco==0 && resc==0
  if ~isstr(TH) && length(TH)==1 % POLAR CAPS
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphaup-%g-%i-%g-%g-%i-%i.mat',TH,L,...
             rnew,rold,sord,blox));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphablup-%g-%i-%i-%g-%g-%i-%i.mat',...
  ...
  elseif ~isstr(TH) && length(TH)==2 % Ring between polar cap angles
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphaup-%g_%g-%i-%g-%g-%i-%i.mat',max(TH),...
             min(TH),L,rnew,rold,sord,blox));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphablup-%g_%g-%i-%i-%g-%g-%i-%i.mat',...
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphaup-%s-%i-%g-%g-%i.mat',h,L,rnew,...
             rold,J));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAUP',...
		     sprintf('glmalphablup-%s-%i-%i-%g-%g-%i.mat',h,L(1),L(2),...
             rnew,rold,J));
    else
     error('The degree range is either one or two numbers')       
    end
...
% 	elseif bp
	  ...
	  [E,Vpp,Np,th,Cp]=sdwcapup(TH,L,m,0,-1,[],[],rnew,rold);
      Vp{mm}=Vpp;
      C{mm}=Cp;
