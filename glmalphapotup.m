...
if upco==0 && resc==0
  if ~isstr(TH) && length(TH)==1 % POLAR CAPS
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphapotup-%g-%i-%g-%g-%i-%i.mat',TH,L,...
             rnew,rold,sord,blox));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphablpotup-%g-%i-%i-%g-%g-%i-%i.mat',...
			     TH,L(1),L(2),rnew,rold,sord,blox));
    else
  ...
  elseif ~isstr(TH) && length(TH)==2 % Ring between polar cap angles
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphapotup-%g_%g-%i-%g-%g-%i-%i.mat',max(TH),...
             min(TH),L,rnew,rold,sord,blox));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphablpotup-%g_%g-%i-%i-%g-%g-%i-%i.mat',...
			     max(TH),min(TH),L(1),L(2),rnew,rold,sord,blox));
    else
      error('The degree range is either one or two numbers')       
    end
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphapotup-%s-%i-%g-%g-%i.mat',h,L,rnew,...
             rold,J));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GLMALPHAPOTUP',...
		     sprintf('glmalphablpotup-%s-%i-%i-%g-%g-%i.mat',h,L(1),L(2),...
             rnew,rold,J));
    else
     error('The degree range is either one or two numbers')       
    end
